...
takeaway: Agent 一啟動，並不會自動看見整顆硬碟；先看工具、工作範圍與你給的權限。
...
scenario:
  ...
  actions:
    - "Agent 先嘗試查看目前可存取的資料夾。"
    - "如果 Downloads 不在工作範圍內，它會停下來、回報看不到，或另外請求權限。"
  result: "你可能看到整理結果，也可能只看到一個權限提示。"
  boundary: "能操作專案資料夾，不代表也能看郵件、相簿、密碼或整顆硬碟。每種工具的範圍都可能不同。"
notTeach: [作業系統權限架構, 容器技術]
---
有些 Agent 只能讀一個專案，有些還能執行指令或連接外部服務。它能做多少事，要看當下環境提供了哪些工具與權限。
