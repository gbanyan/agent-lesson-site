...
context: 第二個練習是把 website/images 的三張原圖各縮小一份到 website/small，原圖保留。實際縮圖的是電腦執行的程式。
...
scenario:
  request: 把 website/images 的三張原圖各縮小一份到 website/small，原圖保留不動。
  actions:
    - Agent 查看電腦是否已有能縮圖的程式；若沒有，可能提出安裝工具或建立一小段程式。
    - 電腦執行選定的程式，讀取原圖並把縮小版本寫到 website/small。
  result: website/images 裡的三張原圖仍在，website/small 多出三張新圖。
