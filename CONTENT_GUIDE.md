...
`src/content/preflight-guides` 是「前置觀念」區的使用前說明：P5 定向「哪些會過期、哪些不會」（先讀），P0 盤點「能請它做什麼」，P1–P4 查產品的錢、裝、選、隱私。這區定位為開始前的定向，可隨需求擴充學習框架類的 guide，不強制固定篇數（lint 不再檢查恰好 5 篇）。正文只描述跨產品仍成立的判斷原則，不寫死價格、方案名稱、安裝指令或版本化模型排行。
